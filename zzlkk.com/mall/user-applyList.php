...
if(isset($_GET['del'])){
	$bsiCore->applyBookingReimburse($bsiCore->ClearInput($_GET['del']));
	header("location:user-applyList.php");
	exit; 
}
$email = "";
if(isset($_GET['email'])){
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function doApply(delid){
			var ans=confirm("温馨提示，你确定申请退款吗？");
			if(ans){
    ...
    <?php include("user-left.php"); ?>
			...
			$html = '<div id="activelistDiv">
					 <h1 style="margin-left:10px;">申请退款</h1><br /><br />
					...
					if(mysql_num_rows($result)){
						...
						$bookingSql = "SELECT booking_id, appmt_id, DATE_FORMAT(checkin_date, '".$bsiCore->userDateFormat."') as checkin, 
						DATE_FORMAT(checkout_date, '".$bsiCore->userDateFormat."')
						as checkout from bsi_bookings where client_id=".$row['client_id']; 
						// and payment_success=false and (CURDATE() > checkout_date OR is_deleted=true) 
						// and is_block=false order by checkout_date";
						$result2 = mysql_query($bookingSql);
						...
						if(mysql_num_rows($result2)){
							...
							while($rowAp = mysql_fetch_assoc($result2)){
								...
								$html .= '<tr>
											...
											<td align="right"><a href="javascript:;" onclick="doApply(\''.$rowAp['booking_id'].'\')">申请退款</a></td>
										</tr>';	
							}
						}
					}
