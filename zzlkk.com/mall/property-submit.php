...
$clientDetails = mysql_fetch_assoc(mysql_query("select * from bsi_clients where client_id=".$_SESSION['apmt_clientid']." and password='".$_SESSION['apmt_password']."'"));
?>
...
<script type="text/javascript">
	...
	function validateAllSteps(){
	   ...
	   if(!isStepValid){
		  $('#wizard').smartWizard('showMessage','请纠正错误的步骤并继续');
	   } else{
	...
	function validateSteps(step){
	  ...
	  if(step == 1){
		 if(validateStep1() == false ){
			isStepValid = false; 
			$('#wizard').smartWizard('showMessage','<font style="color:#F00;">Please correct the errors in step'+step+ ' and click next.</font>');
      ...
      function validateStep2(){
		  ...
		  if(!appmt_type_id && appmt_type_id == ""){
			 isValid = false;
			 $('#msg_appmt_type_id').html('<font style="color:#F00;">&nbsp;&nbsp;请选择房间类型.</font>').show();
		  }else{
		  ...
		  if(!appmt_name && appmt_name.length <= 0){
			 isValid = false;
			 $('#msg_appmt_name').html('<font style="color:#F00;">&nbsp;&nbsp;请输入房间名.</font>').show();
		  }else{
		  ...
		  if(!appmt_type_id && appmt_type_id <= 0){
			 isValid = false;
			 $('#msg_addr1').html('<font style="color:#F00;">&nbsp;&nbsp;请输入房间地址.</font>').show();
		  }else{
			 $('#msg_addr1').html('').show();
			 isValid = true;
		  }
		  ...
		  if(!astate && astate.length <= 0){
			 isValid = false;
			 $('#msg_astate').html('<font style="color:#F00;">&nbsp;&nbsp;请输入省/州.</font>').show();
		  }else{
		  ...
		  if(!azipcode && azipcode.length <= 0){
			 isValid = false;
			 $('#msg_azipcode').html('<font style="color:#F00;">&nbsp;&nbsp;请输入邮编.</font>').show();
		  }else{
	   ...
	   // Validate Email
		  ...
		  if(!emai && emai.length <= 0){
			 ...
			 if( !isValidEmailAddress(emai) ){
				 isValid = false; 
				 $('#msg_email').html('<font style="color:#F00;">请验证 Email Id</font>').show();
			 } else{
				 isValid = true;
			 }
		  }        
		...
		// Validate Username
		   ...
		   if(!un && un.length <= 0){
			 isValid = false;
			 $('#msg_username').html('<font style="color:#F00;">请输入名字</font>').show();
		   }else{
		...
		// validate Last Name
		   ...
		   if(!sname && sname.length <= 0){
			 isValid = false;
			 $('#msg_lastname').html('<font style="color:#F00;">请输入姓氏</font>').show();
		   }else{
		...
		// validate Street Address
		   ...
		   if(!sadd && sadd.length <= 0){
			 isValid = false;
			 $('#msg_sadd').html('<font style="color:#F00;">请输入地址</font>').show();
		   }else{
			 $('#msg_sadd').html('').hide();
		   }
		...
		// validate City
		   ...
		   if(!city && city.length <= 0){
			 isValid = false;
			 $('#msg_city').html('<font style="color:#F00;">请输入地址</font>').show();
		   }else{
			 $('#msg_city').html('').hide();
		   }
		...
		// validate State
		   ...
		   if(!province && province.length <= 0){
			 isValid = false;
			 $('#msg_province').html('<font style="color:#F00;">请填写省/州</font>').show();
		   }else{
...
<script type='text/javascript'>
	...
	myLabel.innerHTML  = remainingChars+" 剩余字符 "+MaxChars
}
//SETUP!!
...
<div id="container-div">
  <div id="container-inside">
    <h1 class="col2">Property Registration</h1>
            ...
            <div id="wizard" class="swMain">
              <ul>
                <li><a href="#step-1">
                  ...
                  <span class="stepDesc"> 用户详情<br />
                  <small>填写你的用户详情</small> </span> </a></li>
                <li><a href="#step-2">
                  <label class="stepNumber">2</label>
                  <span class="stepDesc"> 财产详情<br />
                  <small>填写你的财产详情</small> </span> </a></li>
                <?php if($bsiCore->config['conf_apppmt_listing_type'] == 0){	 ?>
                <li><a href="#step-3">
                  <label class="stepNumber">3</label>
                  <span class="stepDesc"> 确认<br />
                  <small>已完成</small> </span> </a></li>
                <?php }else{ ?>
                <li><a href="#step-3">
                  <label class="stepNumber">3</label>
                  <span class="stepDesc"> 支付<br />
                  <small>已完成</small> </span> </a></li>
                <?php } ?>
              </ul>
              <div id="step-1">
                <h2 class="StepTitle">第一步: 用户详情</h2>
                <table cellpadding="5" cellspacing="2" border="0">
                  <tr>
                    <td align="left"><strong>电子邮箱:</strong></td>
                    <td><input type="text" class="txtBox" name="email" id="email" value="<?php echo $clientDetails['email']; ?>" style="width:250px;" readonly="readonly" />
                      <span id="msg_email"></span></td>
                  </tr>
                  <tr  style="display:none">
                    <td align="left"><strong>First Name:</strong></td>
                    <td><input type="text" class="txtBox"  style="width:250px;" value="<?php echo $clientDetails['first_name']; ?>" name="first_name" id="username" readonly="readonly" />
                      <span id="msg_username"></span></td>
                  </tr>
                  <tr>
                    <td align="left"><strong>真实姓名:</strong></td>
                    <td><input type="text" class="txtBox" style="width:250px;" value="<?php echo $clientDetails['surname']; ?>" name="surname" id="sname" readonly="readonly" />
                    	<span id="msg_lastname"></span></td>
                  </tr>
				  <tr>
                    <td align="left"><strong>省:</strong></td>
                    <td><input type="text" class="txtBox"  style="width:250px;" value="<?php echo $clientDetails['province']; ?>" name="province" id="province" readonly="readonly"/>
                    	<span id="msg_province"></span></td>
                  </tr>
				  <tr>
                    <td align="left"><strong>市:</strong></td>
                    <td><input type="text" class="txtBox"  style="width:250px;" value="<?php echo $clientDetails['city']; ?>" name="city" id="city" readonly="readonly" />
                    	<span id="msg_city"></span></td>
                  </tr>
                  <tr  style="display:none">
                    <td align="left"><strong>Country:</strong></td>
                    <td><input type="text" class="txtBox"  style="width:250px;" value="<?php echo $clientDetails['country']; ?>" name="country" id="country" readonly="readonly"/>
                    	<span id="msg_country"></span></td>
                  </tr>
                  <tr>
                    <td align="left"><strong>详细地址:</strong></td>
                    <td><input type="text" class="txtBox" style="width:250px;" value="<?php echo $clientDetails['street_addr']; ?>" name="street_addr" id="sadd" readonly="readonly" />
                    	<span id="msg_sadd"></span></td>
                  </tr>
                  <tr  style="display:none">
                    <td align="left"><strong>Street Address2:</strong></td>
                    <td><input type="text" class="txtBox" style="width:250px;" value="<?php echo $clientDetails['street_addr2']; ?>" name="street_addr2" id="sadd2" readonly="readonly" /></td>
                  </tr>
                  <tr   style="display:none">
                    <td align="left"><strong>邮编:</strong></td>
                    <td><input type="text" class="txtBox"  style="width:250px;" value="<?php echo $clientDetails['zip']; ?>" name="zip" id="zip" readonly="readonly"/>
                    	<span id="msg_zip"></span></td>
                  </tr>
                  <tr>
                    <td align="left"><strong>电话号码:</strong></td>
                    <td><input type="text" class="txtBox"  style="width:250px;" value="<?php echo $clientDetails['phone']; ?>" name="phone" id="phone" readonly="readonly"/>
              ...
              <div id="step-3">
                <?php if($bsiCore->config['conf_apppmt_listing_type']==0){	 ?>
                <h2 class="StepTitle">第三步: 确认</h2>
                <table cellspacing="3" cellpadding="3" align="left" width="100%">
                  <tr>
                    <td align="left"><textarea class="txtBox" rows="3" style="width:100%; height:300px;" readonly="readonly">
					Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, 
					totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo.
					Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit,
					sed quia Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, 
					totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo.
					Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit,
					sed quia Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium,
					totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. 
                  ...
                  <tr>
                    <td align="left"><input type="checkbox" name="tos" id="tos1" />
                      请接受以上的条款和条件.&nbsp;&nbsp;<span id="msg_tos1"></span></td>
                  </tr>
                </table>
                <?php }else{ ?>
