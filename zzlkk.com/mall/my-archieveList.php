			...
			$html = '<div id="activelistDiv">
					...
					if(mysql_num_rows($result)){
						...
						while($row = mysql_fetch_assoc($result)){
							...
							$result2 = mysql_query("SELECT booking_id, appmt_id, DATE_FORMAT(checkin_date, '".$bsiCore->userDateFormat."') as checkin, DATE_FORMAT(checkout_date, '".$bsiCore->userDateFormat."') as checkout from bsi_bookings where appmt_id=".$row['appmt_id']." and payment_success=true and (CURDATE() > checkout_date OR is_deleted=true)  and is_block=false order by checkout_date");
