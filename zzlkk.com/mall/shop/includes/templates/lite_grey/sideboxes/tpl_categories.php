...
/*	  $content .= '<div class="categories-top-list">';*/
	  if ($new_style == 'category-products' || $new_style == 'category-subs') {
			if($i==($tempIndex+1)){
				$open_tag = '<div style="display:none;" class="child"><div class="subcategory" rel="'.$i.'">';
			}else{
				$open_tag = '<div class="subcategory" rel="'.$i.'">';
			}
			$beforeIndex = $i;
	  } else {
			$tempIndex = $i;
			if($i==($beforeIndex+1)){
				$open_tag = '</div><div class="categories-top-list ' . $top . '">';
			}else{
	  ...
	  /*#### Wel*/
	  if ($new_style == 'category-products' || $new_style == 'category-subs') {
			// if($i==($tempIndex-1)&&($tempIndex-1)!=0){
				// $close_tag = '</div></div>';
			// }else{
...
// display a separator between categories and links
  ...
  $content .= '<script type="text/javascript">
					$(function(){
						$(".categories-top-list").hover(function(){
							$(this).next(".child").show(500);
						},function(){
							
							//$(this).next(".child").hide(500);
						});
						$(".categories-top-list").click(function(){
