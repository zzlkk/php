...
// class constructor
    function moneyorder() {
      ...
      if (IS_ADMIN_FLAG === true && (MODULE_PAYMENT_MONEYORDER_ACCOUNT == '店主或网店名称' || MODULE_PAYMENT_MONEYORDER_PAYTO == '')) $this->title .= '<span class="alert"> (未正确设置银行帐号)</span>';
...
// class methods
    ...
    function install() {
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('打开现金支票模块', 'MODULE_PAYMENT_MONEYORDER_STATUS', 'True', '您要使用现金支票模块吗?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now());");
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('显示顺序', 'MODULE_PAYMENT_MONEYORDER_SORT_ORDER', '0', '显示顺序：小的显示在前。', '6', '0', now())");
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('设置订单状态', 'MODULE_PAYMENT_MONEYORDER_ORDER_STATUS_ID', '0', '设置通过该支付方式付款的订单状态', '6', '0', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
    }
