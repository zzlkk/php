...
if (isset($_GET['action']) && ($_GET['action'] == 'process')) {
  ...
  */
    ...
    $check_customer_query = "SELECT customers_id, customers_firstname, customers_lastname, customers_password,
                                    customers_email_address,customers_telephone, customers_default_address_id,
    ...
    if (!$check_customer->RecordCount()) {
      ...
      if (!zen_validate_password($password, $check_customer->fields['customers_password'])) {
        ...
        $_SESSION['customer_telephone'] = $check_customer->fields['customers_telephone'];
        $_SESSION['customer_email'] = $check_customer->fields['customers_email_address'];
		...
		if (isset($_GET['f'])&&$_GET['f']=="wowo"){
			echo json_encode(array("errorcode"=>0));
			exit;
		}elseif (sizeof($_SESSION['navigation']->snapshot) > 0) {
