...
<div class="content">
<div class="border"><table id="rlist">
<tr><th>名称</th><th>键值</th><th>条件</th><th>排序</th><th>操作 <a class="button" href="<?php echo U('/Role/index');?>">刷新</a> <a class="button" href="javascript:add(0)">添加</a></th></tr>
<?php if(is_array($rules)): foreach($rules as $key=>$c): ?><tr id="cr<?php echo ($c["id"]); ?>"><td><?php echo ($c["indent"]); echo ($c["title"]); ?></td><td><?php echo ($c["name"]); ?></td><td><?php echo ($c["condition"]); ?></td><td><?php echo ($c["sort"]); ?></td>
	<td><a class="button" href="<?php echo U('/Role/save?status='.(1-$c['status']).'&id='.$c['id']);?>"><?php echo ($c['status']?'禁用':'启用'); ?></a>
...
<div id="cwin" style="display:none;width:400px;height:168px;background:#fff;padding-top:20px;border:10px solid #eee;">
	<form action="<?php echo U('/Role/save');?>" method="post"><div style="margin:0 20px 0 90px;">
