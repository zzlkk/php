...
<div class="border">
<table id="rlist">
<tr><th>名称</th><th>键值</th><th>条件</th><th>排序</th><th>操作 <a class="button" href="<?php echo U('/User/index');?>">刷新</a> <a class="button" href="javascript:add(0)">添加</a></th></tr>
<?php if(is_array($users)): foreach($users as $key=>$c): ?><tr id="cr<?php echo ($c["id"]); ?>"><td><img src="<?php echo ($c["head"]); ?>" height="22" alt="" /><?php echo ($c["nick"]); ?></td><td><?php echo ($c["email"]); ?></td><td><?php echo ($r["phone"]); ?></td><td><?php echo ($r["regTime"]); ?></td>
	<td><a class="button" href="<?php echo U('/User/save?status='.(1-$c['status']).'&id='.$c['id']);?>"><?php echo ($c['status']?'禁用':'启用'); ?></a>
		<a class="button" href="javascript:edit(<?php echo ($c['id']); ?>)">编辑</a>
		<a class="button" onclick="return confirm('所有子项也将一并删除！确定删除[<?php echo ($c['nick']); ?>]？')" href="javascript:del(<?php echo ($c['id']); ?>)">删除</a>
...
<div id="cwin" style="display:none;width:400px;height:168px;background:#fff;padding-top:20px;border:10px solid #eee;">
	<form action="<?php echo U('/User/save');?>" method="post"><div style="margin:0 20px 0 90px;">
...
function del(id){
	if(!isNaN(id)&&id>0){
		$.post("<?php echo U('/Role/delete');?>",{id:id},function(data){
...
function edit(id){
	if(!isNaN(id)){
		$.post("<?php echo U('/Role/edit');?>",{id:id},function(data){
			if(data){
