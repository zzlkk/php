...
class UserAction extends BaseAction{
	public function index(){
		...
		$model=M('users');
		$Page = new Page($model->count(),$size);
		$this->users = $model->limit($Page->firstRow,$Page->listRows)->select();
	...
	public function edit(){
		$id=I('id',0,'intval');
		if($id>0){
			$this->ajaxReturn(M('users')->find($id));
		}else{
	...
	public function delete(){
		$id=I('id',0,'intval');
		$this->ajaxReturn(parent::delete('users',$id,true));
	}
}
?>